...
public abstract class RPSAbstract implements RPSInterface {
    ...
    public boolean isValidMove(String move) {
        for(int i=0;i<possibleMoves.length;i++){
            if (possibleMoves[i].equalsIgnoreCase(move)){
                return true;
            }
        }
        return false;  // dummy return value so code compiles.
    }
    public int returnPos(String[]strarray, String word){
    ...
    public void play(String playerMove, String cpuMove) {
        int answer = determineWinner(playerMove, cpuMove);
        if(answer==INVALID_INPUT_OUTCOME){
            System.out.println(INVALID_INPUT);
            numGames-=1;
        }
        else if(answer==CPU_WIN_OUTCOME){
            System.out.println(CPU_MOVE + " " + cpuMove + ". " + CPU_WIN);
            cpuMoves[numGames]=cpuMove;
            playerMoves[numGames]=playerMove;
            numCPUWins+=1;
        }
        else if(answer==PLAYER_WIN_OUTCOME){
            System.out.println(CPU_MOVE + " " + cpuMove+ ". " + PLAYER_WIN);
            cpuMoves[numGames]=cpuMove;
            playerMoves[numGames]=playerMove;
            numPlayerWins+=1;
        }
        else if(answer==TIE_OUTCOME){
